...
Ôn tập bài thi tổng hợp sử dụng `BottomNavigationView` với custom vector icon điều hướng giữa 5 Fragment: **Home** (animation), **Đổi đơn vị** (m ↔ km, °C ↔ °F), **BMI** (tính chỉ số BMI + phân loại + lời khuyên), **Danh lam** (RecyclerView 8 cảnh quan + AlertDialog), **Sách** (SQLite + RecyclerView + tìm kiếm realtime).
